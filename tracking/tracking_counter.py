...
class RackTracker:
   ...
   """
    ...
    def N_Pholders(self) -> int:
        total_boxes = sum(value[0] * value[1] for key, value in CONSTANTS.NUMBER_BOXES_PER_SHELVE[self.rack_name].items())
        return total_boxes - (self.N_full_KLT + self.N_empty_KLT)

    @property
    def shelf_N_Pholders(self) -> Dict[int, int]:
        return {f"shelf_{shelf}": self.placeholders_per_shelf(shelf) for shelf, value in CONSTANTS.NUMBER_BOXES_PER_SHELVE[self.rack_name].items() if self.placeholders_per_shelf(shelf) > 0 }
    
    def placeholders_per_shelf(self, shelf: int) -> int:
        """returns the number of placeholders in a shelf"""
        total_boxes_in_shelf = CONSTANTS.NUMBER_BOXES_PER_SHELVE[self.rack_name][shelf][0] * CONSTANTS.NUMBER_BOXES_PER_SHELVE[self.rack_name][shelf][1]
        n_placeholder_in_shelf = total_boxes_in_shelf - (self.shelves[shelf]["N_full_KLT"] + self.shelves[shelf]["N_empty_KLT"])
...
def create_submission_dict(scanned_racks: List[RackTracker], maP: float, fps: int) -> Dict[str, Any]:
    submission_dict = {"eval_video": [],
            "maP": maP,
            "FPS": fps}
    for scanned_rack in scanned_racks:
        video_entry = {"rack_name": scanned_rack.rack_name, 
                       "rack_conf": scanned_rack.rack_conf, 
                        "N_full_KLT": scanned_rack.N_full_KLT,
                        "N_empty_KLT": scanned_rack.N_empty_KLT, 
                        "N_Pholders": scanned_rack.N_Pholders,
